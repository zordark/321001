...
namespace BugRacer
{
    public partial class BugRacerForm : Form
    {
        ...
        {
            if (listBox1.Items.Count < 5) listBox1.Items.Add("Остальные воздержались от ставок");
            timer1.Enabled = true;
        ...
        private void MakeBet(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(textBox1.Text, out amount))
                if(amount == 0)
                    MessageBox.Show("Ставки начинаются от 1 рубля");
                else if(currentGambler.HasEnoughMoney(amount))
                {
                    ...
                    MessageBox.Show("У игрока не хватает денег для ставки");
            else 
        ...
        private void BugRacerForm_Load(object sender, EventArgs e)
        {
